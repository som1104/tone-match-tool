    ...
    <div style={{ marginTop: 16 }}>
      {regionActive ? (
        <p style={{ fontSize: 12, color: "var(--text-secondary)", margin: "0 0 12px" }}>
          영역별로 나눠 보정 중이에요. 조정은 아래 <strong>영역별로 나눠 보정</strong> 버튼에서 계속할 수 있어요.
        </p>
      ) : (
        <div style={{ display: "flex", alignItems: "flex-end", gap: 24, flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6 }}>
            <button
              ...
              style={{ fontSize: 12, visibility: overridden ? "visible" : "hidden", pointerEvents: overridden ? "auto" : "none" }}
            >
              기본값으로
            </button>
            <button onClick={onApplyToAll} style={{ fontSize: 12 }}>이 값을 전체에 적용</button>
          </div>
        </div>
      )}

      <div style={{ display: "flex", gap: 8, marginTop: 14, paddingTop: 14, borderTop: "0.5px solid var(--border)" }}>
          ...
          {regionActive ? (
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10, flexWrap: "wrap", gap: 8 }}>
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  <button onClick={onStartRegionPick} style={{ fontSize: 12 }}>
                    {regionPickMode ? "선택 완료" : "영역 추가 선택"}
                  </button>
                  {region && <button onClick={onUndoRegionPick} style={{ fontSize: 12 }}>되돌리기</button>}
                  {region && <button onClick={onClearRegion} style={{ fontSize: 12 }}>영역 삭제</button>}
                  {region && <button onClick={onApplyRegionToAll} style={{ fontSize: 12 }}>이 영역을 전체에 적용</button>}
                </div>
              </div>
              ...
              {/* 영역을 아직 안 잡았을 때도 아래 슬라이더 블록을 미리 같은 높이로 확보해둔다
                  ...
                  늘어나는 걸 막기 위한 처리다. */}
              <p style={{ fontSize: 12, color: "var(--text-muted)", margin: "0 0 8px", visibility: region ? "hidden" : "visible" }}>
                아직 선택한 영역이 없어요. 비교 화면을 클릭해보세요.
              </p>
              <div style={{ display: "flex", flexDirection: "column", gap: 4, opacity: region ? 1 : 0.45, pointerEvents: region ? "auto" : "none" }}>
                <p style={{ fontSize: 12, fontWeight: 600, margin: "0 0 2px" }}>선택 영역</p>
                ...
                <p style={{ fontSize: 12, fontWeight: 600, margin: "8px 0 2px" }}>나머지</p>
            ...
            <div>
              <p style={{ fontSize: 12, color: "var(--text-muted)", margin: "0 0 8px" }}>
                영역을 선택하면 선택 영역과 나머지를 따로 조정할 수 있어요.
              </p>
              <button onClick={onStartRegionPick} style={{ fontSize: 12 }}>
                영역 선택 시작
              </button>
        ...
        <Popover label={`밝기 양 끝 보호${protectEnabled ? " · 사용 중" : ""}`} width={340}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: protectEnabled ? 10 : 4, gap: 12 }}>
            <p style={{ fontSize: 11, color: "var(--text-muted)", margin: 0 }}>
              모든 이미지에 공통 적용돼요. 검정(선화)·흰색(배경)에 가까울수록 보정을 원래 색으로 되돌려요.
            </p>
            <button
              className={protectEnabled ? "primary" : undefined}
